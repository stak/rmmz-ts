...
export class Game_Character extends Game_CharacterBase {
  ...
  _moveRoute: MZ.EventMoveRoute | null = null;
  _moveRouteIndex = 0;
  _originalMoveRoute: MZ.EventMoveRoute | null = null;
  ...
  setMoveRoute(moveRoute: MZ.EventMoveRoute): void {
  ...
  processMoveCommand(command: MZ.EventMoveCommand): void {
  ...
  moveRandom(): void {
    const d = 2 + Math.randomInt(4) * 2 as MZ.MoveDirection;
  ...
  turnRandom(): void {
    this.setDirection(2 + Math.randomInt(4) * 2 as MZ.MoveDirection);
  };
  ...
  findDirectionTo(goalX: number, goalY: number): MZ.MoveDirection {
    ...
    while (nodeList.length > 0) {
        ...
        for (let j = 0; j < 4; j++) {
            const direction = 2 + j * 2 as MZ.MoveDirection;
